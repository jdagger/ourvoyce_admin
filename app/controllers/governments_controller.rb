class GovernmentsController < ApplicationController
  ...
  def create
    ...
    if @government.save
      redirect_to(government_path(@government), :notice => 'Government was successfully created.')
    else
      render :action => "new"
    end
  end
  ...
  def update
    ...
    if @government.update_attributes(params[:government])
      redirect_to(government_path(@government), :notice => 'Government was successfully updated.')
    else
      render :action => "edit"
    end
  end
